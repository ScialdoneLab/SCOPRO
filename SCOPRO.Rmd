...
# Preprocess mouse in vivo dataset
...
preprocess_mouse_published_data <- function(raw_count_mohammed, stages_mohammed, raw_count_deng, stages_deng, convert_name_id_mouse){
  stages_mohammed_updates <- stages_mohammed[stages_mohammed != "E3.5"&stages_mohammed != "E6.75"]
...
common_new <- common[isUnique(name)]
...
cluster_mouse <- c(cluster_mohammed_updates, stages_deng)
...
```{r}
setwd("/Users/gabriele.lubatti/Desktop/Phd/Early_embryonic_like_cells/Mouse/Mouse_published_dataset/Mohammed")



load(file="raw_count_mohammed_updates.Rda")
load(file="stages_mohammed.Rda")
stages_mohammed_updates=stages_mohammed[stages_mohammed!="E3.5"&stages_mohammed!="E6.75"]
load(file="cluster_mohammed_updates.Rda")
...
raw_counts_deng=raw_counts_deng[,(stages_deng_update!="fibroblast")&(stages_deng_update!="liver")&(stages_deng_update!="Zygote")&(stages_deng_update!="2_cell")&(stages_deng_update!="Mid_2_cell")]
stages_deng_update=stages_deng_update[(stages_deng_update!="fibroblast")&(stages_deng_update!="liver")&(stages_deng_update!="Zygote")&(stages_deng_update!="2_cell")&(stages_deng_update!="Mid_2_cell")]
#save(raw_count_mohammed,file="raw_count_mohammed.Rda")
#save(stages_mohammed,file="stages_mohammed.Rda")

#stages_deng=paste0(stages_deng,"_deng")
...
common=intersect(row.names(raw_count_mohammed_updates),row.names(genome_mouse))


nomi=genome_mouse[common,3]

common_new=common[isUnique(nomi)]

nomi_new=genome_mouse[common_new,3]

raw_count_mohammed_new=raw_count_mohammed_updates[common_new,]
row.names(raw_count_mohammed_new)=nomi_new
...
```{r}
common_mouse=Reduce(intersect, list(row.names(raw_count_mohammed_new),row.names(raw_counts_deng)))
final_name_pig=common_mouse
raw_count_mohammed_new_new=raw_count_mohammed_new[final_name_pig,]
raw_counts_deng=raw_counts_deng[final_name_pig,]
raw_counts_mouse_all=cbind(raw_count_mohammed_new_new,raw_counts_deng)
row.names(raw_counts_mouse_all)=final_name_pig

stages_mouse=c(stages_mohammed_updates,stages_deng_update)
cluster_mouse=c(cluster_mohammed_updates,stages_deng_update)
```


# Stop preprocess mouse in vivo dataset
# SCOPRO
...
```{r}
markers_to_plot = all_genes
    label_1 = "Mouse ESC"
    label_2 = "Mouse vitro"
    norm_vitro = norm_vitro
    norm_vivo = norm_vivo_small[,cluster_mouse_published_small=="Late_2_cell"]
    
    cluster_vivo = cluster_mouse_published_small[cluster_mouse_published_small=="Late_2_cell"]
    final_name = all_genes_label
    max_size = 9
    text_size = 9.5
    title_name = "Late_2_cell"
    norm_expression = FALSE
    
plot_score_genes = function (markers_to_plot, label_1, label_2, norm_vitro, norm_vivo, 
    cluster_vitro, cluster_vivo, final_name, max_size = 9, text_size = 9.5, 
    title_name, norm_expression = FALSE) 
{
    markers_to_plot <- markers_to_plot[markers_to_plot %in% row.names(norm_vitro)]
    all_markers_common_name <- final_name
    label_mouse <- c(rep(label_1, length(colnames(norm_vitro))), 
        rep(label_2, length(colnames(norm_vivo))))
    mouse_norm_common = cbind(norm_vitro[markers_to_plot, ], 
        norm_vivo[markers_to_plot, ])
    cluster_vitro_name <- paste(label_1, factor(cluster_vitro), 
        sep = "-")
    cluster_vivo_name <- paste(label_2, factor(cluster_vivo), 
        sep = "-")
    mouse_norm_common <- cbind(norm_vitro[markers_to_plot, ], 
        norm_vivo[markers_to_plot, ])
    label_mouse <- c(cluster_vitro_name, cluster_vivo_name)
    ...
    out_2 <- plot_balons(mouse_norm_common, label_mouse, markers_to_plot, 
        length(markers_to_plot), length(colnames(mouse_norm_common)), 
        max_size = max_size, text_size = text_size, all_markers_common_name, 
        keep_order = FALSE, norm_expression = T)
...
plot_balons = function (norm_counts, final_cluster, genes_to_plot, max_number, 
    total_cells, max_size = 5, text_size = 7, label_final, keep_order = TRUE, 
    norm_expression = FALSE) 
{
    fattore <- factor(final_cluster, levels = unique(final_cluster))
  ...
  return(p)
    ...
    if (norm_expression == TRUE) {
        ...
        all_markers_values <- rep(list(0), length(livelli))
        for (i in 1:length(livelli)) {
            gene_values_0 <- apply(norm_counts[genes_to_plot, 
                final_cluster == livelli[i]], 1, mean)
        ...
        p <- ggplot2::ggplot(balloon_melted, ggplot2::aes(x = factor(cluster_label_all, 
            ...
            size = "Value") + ggplot2::xlab("Cluster") + ggplot2::ylab("Markers") + 
...
mouse_plot=plot_score_genes(all_genes,"Mouse ESC","Mouse vitro", norm_vitro,norm_vivo_small[,cluster_mouse_published_small=="Late_2_cell"],cluster_vitro,cluster_mouse_published_small[cluster_mouse_published_small=="Late_2_cell"],all_genes_label,7,10,"Late_2_cell", norm_expression = TRUE)

mouse_plot


mouse_plot=plot_score_genes(all_genes,"Mouse ESC","Mouse vitro", norm_vitro,norm_vivo_small[,cluster_mouse_published_small=="Late_2_cell"],cluster_vitro,cluster_mouse_published_small[cluster_mouse_published_small=="Late_2_cell"],all_genes_label,7,10,"Late_2_cell", norm_expression = FALSE)
...
##New function distance vivo/ vitro
...
difference_significance_vivo_single <- function(norm_vivo, mean_profile_vivo, mean_profile_vitro, p_value, name_vivo, genes_all, cluster_vivo, selected_stages_vitro){
  ...
  orm_vitro_4_4 <- cbind(norm_vitro_4,mean_profile_vitro_cluster)
  dissimilarity_4_4 <- sqrt((1-cor((norm_vitro_4_4), method = "spearman"))/2)
...
```{r }
difference_significance_vitro_new <- function(norm_vitro, p_value = 0.10, marker_stages, marker_stages_filter, cluster_vitro, cluster_vivo, selected_stages, norm_vivo){
  for (i in selected_stages){
    index_specific <- which(selected_stages %in% i)
    marker_specific <- marker_stages[[index_specific]]
    if (length(marker_specific) == 0) {
  ...
  mean_profile_vivo <- SCOPRO:::find_mean_cluster(cluster_vivo, selected_stages_vivo, norm_vivo, genes_all, method = "mean")
  selected_stages_vitro <- levels(factor(cluster_vitro))
  mean_profile_vitro <- SCOPRO:::find_mean_cluster(cluster_vitro, selected_stages_vitro, norm_vitro, genes_all, method = "mean")
name_vitro <- levels(factor(cluster_vitro))
difference_vitro <- rep(list(0),length(name_vitro))
...
```{r }
plot_distance_vivo_vitro_new <- function(norm_vivo, cluster_vivo, selected_stages, marker_stages, marker_stages_filter, norm_vitro, cluster_vitro, text_size = 10, show_significance = FALSE, p_value = 0.10){
...
difference_vitro_vivo <-
  difference_significance_vitro_new(norm_vitro,p_value,marker_stages, marker_stages_filter, cluster_vitro, cluster_vivo, selected_stages, norm_vivo)
...
```{r }
plot_distance_vivo_vitro_new(norm_vivo_small, cluster_mouse_published_small, selected_stages, marker_stages, marker_stages_filter, norm_vitro, cluster_vitro, text_size = 10, show_significance = TRUE, p_value = 0)

plot_distance_vivo_vitro_new(norm_vivo_small, cluster_mouse_published_small, selected_stages, marker_stages, marker_stages_filter, norm_vitro, cluster_vitro, text_size = 10, show_significance = FALSE, p_value = 0.05)


difference_vitro_vivo = difference_significance_vitro_new(norm_vitro,p_value = 0.001,marker_stages, marker_stages_filter, cluster_vitro, cluster_mouse_published_small, selected_stages, norm_vivo_small)
...
find_median_cluster = function (cluster_vivo, selected_stages, norm_vivo, genes) 
{
    ...
    for (i in 1:length(number_cluster)) {
        median_es[[i]] <- apply(norm_vivo[genes, cluster_vivo == 
            number_cluster[i]], 1, "median")
    }
    return(median_es)
}
...
plot_score_genes_heatmap <- function(norm_vivo, cluster_vivo, norm_vitro, cluster_vitro, marker_plot, marker_plot_name, mean_profile = FALSE, color_vector = NULL){
  ...
  if (mean_profile == TRUE){
    ...
    row.names(heatdata) <- all_genes_label
...
```{r }
plot_score_genes_heatmap(norm_vivo_small[,cluster_mouse_published_small=="Late_2_cell"], cluster_mouse_published_small[cluster_mouse_published_small == "Late_2_cell"], norm_vitro, cluster_vitro, all_genes, all_genes_label, mean_profile = FALSE, color_vector = NULL)
```


```{r }
plot_score_genes_heatmap(norm_vivo_small[,cluster_mouse_published_small=="Late_2_cell"], cluster_mouse_published_small[cluster_mouse_published_small == "Late_2_cell"], norm_vitro, cluster_vitro, all_genes, all_genes_label, mean_profile = TRUE, color_vector = NULL)
```
...
```{r }
SCOPRO::plot_umap(cordinate_umap_0, cluster_vitro)

```
```{r }
SCOPRO::plot_gene(norm_vitro, cordinate_umap_0,"Zscan4c","Zscan4c")
...
```{r }
heatmap_markers_vivo(norm_vivo_small, marker_stages, selected_stages, max_number =10, cluster_mouse_published_small, median_profile = FALSE, color_vector = NULL)
```
```{r }
...
heatmap_markers_vitro(norm_vitro,common_genes, cluster_vitro, mean_profile = FALSE,color_vector = NULL)
```

```{r }

heatmap_markers_vitro(norm_vitro,common_genes, cluster_vitro, mean_profile = TRUE,color_vector = NULL)
```

```{r }
heatmap_markers_vivo_median_profile(norm_vivo_small, marker_stages, selected_stages, max_number =10, cluster_mouse_published_small, color_vector = NULL)
```
...
plot_barplot <- function(obj, method, cluster_vitro, thresold, fraction = TRUE){
  if (method == "Seurat") {
    ...
    return(p[[1]]+ggplot2::theme(text = element_text(size=14),axis.text.x = element_text(size=14,angle = 45, vjust = 1, hjust = 1)))
    }
  if (method == "scibetR") {
...
```
```{r}
plot_boxplot_seurat=function (gene_expr, cluster_vivo, 
    title_name, fill_name, x_name, y_name) 
{
    data_common <- data.frame(cluster_vivo, gene_expr)
    ggplot2::ggplot(data_common, ggplot2::aes(x = cluster_vivo, 
        y = gene_expr, fill = cluster_vivo)) + ggplot2::geom_boxplot() + 
        ggplot2::geom_jitter(position = ggplot2::position_jitter(0.2)) + 
        ggplot2::xlab(x_name) + ggplot2::ylab(y_name) + ggplot2::ggtitle(title_name) + 
...
seurat_score_2cell=mayra_seurat_0_seurat$prediction.score.Late_2cell

plot_boxplot_seurat( seurat_score_2cell, cluster_vitro,"Seurat score late 2 cells" ,"Stage", "Cluster", "Log norm")
...
plot_barplot(mayra_seurat_0_scibet, method = "scibetR", cluster_vitro, threshold = 0.1, fraction = TRUE)
plot_barplot(mayra_seurat_0_scibet, method = "scibetR", cluster_vitro, threshold = 0.1, fraction = FALSE)


plot_barplot(mayra_seurat_0_seurat, method = "Seurat", cluster_vitro, threshold = 0.1, fraction = TRUE)
plot_barplot(mayra_seurat_0_seurat, method = "Seurat", cluster_vitro, threshold = 0.1, fraction = FALSE)


plot_barplot(mayra_seurat_0_scmap, method = "scmap", cluster_vitro, threshold = 0.1, fraction = TRUE)
plot_barplot(mayra_seurat_0_scmap, method = "scmap", cluster_vitro, threshold = 0.1, fraction = FALSE)
...
seurat_score_2cell=mayra_seurat_0_seurat$prediction.score.epi_4.5

plot_boxplot_seurat( seurat_score_2cell, cluster_vitro,"Seurat score epi 4.5" ,"Stage", "Cluster", "Log norm")
...
seurat_score_2cell=mayra_seurat_0_seurat$prediction.score.epi_5.5

plot_boxplot_seurat( seurat_score_2cell, cluster_vitro,"Seurat score epi 5.5" ,"Stage", "Cluster", "Log norm")

```

## FROM NOW INTILL THE END OF THE SCRIPT NOT USEFULL ANY MORE
...
## Test if the code written in the readme file on the github repo works.
